...
public class Test09 {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < lottoList.length; i++) {
			lottoList[i] = r.nextInt(6) + 1;
			System.out.println("주사위 번호: " + lottoList[i]);
			}
		
		for(int i = 0; i < lottoList.length; i++) {
			if (lottoList[i] == 1) {
				count1++;
			} else if (lottoList[i] == 2) {
				count2++;
			} else if (lottoList[i] == 3) {
				count3++;
			} else if (lottoList[i] == 4) {
				count4++;
			} else if (lottoList[i] == 5) {
				count5++;
			} else if (lottoList[i] == 6) {
				count6++;
			}
		}
