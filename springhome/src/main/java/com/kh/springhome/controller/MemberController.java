...
public class MemberController {
	...
	@GetMapping("/join") //jsp로
	public String join() {
		return "/WEB-INF/views/member/join.jsp";
		
	}
	
	@PostMapping("/join") //jsp로
...
//		return "redirect:/member/joinFinish"; //절대경로
	...
	@RequestMapping("/joinFinish") //redirect로 
	...
	컨트롤러에서 선언만 하면 사용할 수 있으며, key=value 형태로 저장된다.
	
	[1] 추가 - session.setAttribute("key", value); 로그인과 연관
	[2] 확인 - session.getAttribute("key"); 로그인 상태 
	[3] 삭제 - session.removeAttribute("key"); 로그아웃과 연관
	...
	public String login(@ModelAttribute MemberDto inputDto, HttpSession session) {
		...
		//equals. ignorecase 쓰지 말기 대소문자 구분
...
//				return "redirect:오류페이지";
			...
			//세션에 아이디 + 등급 저장
			session.setAttribute("name", findDto.getMemberId()); //로그인 최종 처리코드
